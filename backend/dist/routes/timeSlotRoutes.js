...
router.get("/available", auth_1.auth, async (req, res) => {
    try {
        const { area, date } = req.query;
        if (!area || !date) {
            return res.status(400).json({ message: "Area and date are required" });
        }
        const availableSlots = await TimeSlot_1.default.findAvailableSlots(area, new Date(date));
        // Get AI predictions for optimal time slots
        const aiServiceHealthy = await (0, aiService_1.checkAIServiceHealth)();
...
// Book a time slot
router.post("/book", auth_1.auth, async (req, res) => {
    try {
        ...
        if (slot.postmanId) {
            const postmanDeliveries = await Order_1.default.countDocuments({
                timeSlotId: slotId,
...
router.get("/distribution", auth_1.auth, async (req, res) => {
    try {
        ...
        const distribution = await Order_1.default.aggregate([
            { $match: query },
            {
                $group: {
                    _id: "$timeSlotId",
...
router.get("/efficiency", auth_1.auth, async (req, res) => {
    try {
        if (req.user.role !== "admin") {
            return res.status(401).json({ message: "Not authorized" });
        }
        ...
        const efficiencyMetrics = await Order_1.default.aggregate([
            { $match: query },
            {
                $lookup: {
                    from: "timeslots",
...
router.post("/admin/slots", auth_1.auth, async (req, res) => {
    try {
        if (req.user.role !== "admin") {
            return res.status(401).json({ message: "Not authorized" });
        }
        ...
        const result = await TimeSlot_1.default.bulkWrite(operations);
        res.json({
